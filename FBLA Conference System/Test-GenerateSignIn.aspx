<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Test-GenerateSignIn.aspx.cs" Inherits="FBLA_Conference_System.Test_GenerateSignIn" %>
...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <table style="margin: 0 0 20px 0;padding:0" cellpadding="0" cellspacing="0"><tr valign="middle">
        <td><h2>Generate Sign-In Accounts for Online Testing</h2></td><td style="padding-left:20px">
    ...
    <asp:UpdatePanel ID="updGenerateSignIns" runat="server"><ContentTemplate>
        ...
        <asp:GridView ID="gvSignIns" runat="server" DataSourceID="sqlSignIns"
            ...
            <EmptyDataTemplate>
                <div style="padding-top:15px;border-top:1px solid #808080;font-weight:bold;font-style:italic">No student testing accounts have been created for this conference.</div>
            </EmptyDataTemplate>
            <Columns>
                <asp:BoundField DataField="RegionName" HeaderText="Region" SortExpression="RegionName,ChapterName,LastName,FirstName"
                    ItemStyle-Width="250px" HeaderStyle-HorizontalAlign="Left" />
                <asp:BoundField DataField="ChapterName" HeaderText="Chapter" SortExpression="ChapterName,LastName,FirstName"
                ...
                <asp:BoundField DataField="LastName" HeaderText="Last Name" SortExpression="LastName,FirstName,RegionName,ChapterName"
                    ItemStyle-Width="100px" HeaderStyle-HorizontalAlign="Left" />
                <asp:BoundField DataField="TestUsername" HeaderText="SignIn Name" SortExpression="TestUsername,RegionName,ChapterName"
                    ItemStyle-Width="100px" HeaderStyle-HorizontalAlign="Left" />
                <asp:BoundField DataField="TestPassword" HeaderText="Password" SortExpression="TestPassword"
                    ItemStyle-Width="100px" HeaderStyle-HorizontalAlign="Left" />
            </Columns>
    ...
    <asp:SqlDataSource ID="sqlSignIns" runat="server" ConnectionString="<%$ ConnectionStrings:ConfDB %>" 
        SelectCommand="SELECT RegionName, ChapterName, FirstName, LastName, TestUsername, TestPassword FROM TestCredentials TC INNER JOIN NationalMembers M ON TC.MemberID=M.MemberID INNER JOIN Chapters C ON M.ChapterID=C.ChapterID INNER JOIN Regions R ON C.RegionID=R.RegionID WHERE ConferenceID=@ConferenceID ORDER BY RegionName,ChapterName,LastName,FirstName">
        <SelectParameters>
